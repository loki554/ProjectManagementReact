...
public record CreateTaskRequest(
        ...
        UUID sprintId,
        /*
         * Шаблон, по которому заводится задача (4.13). Единственное, что сервер берёт из
         * него сам, — чек-лист: остальные поля шаблона к этому моменту уже подставлены в
         * форму и приехали выше как обычные значения, отредактированные человеком. Поэтому
         * шаблон здесь — не «создать по шаблону», а «донести то, чего в форме нет»; см.
         * TaskService.applyTemplateChecklist.
         */
        UUID templateId
) {
}
